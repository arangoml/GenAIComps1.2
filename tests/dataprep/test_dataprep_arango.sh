...
export LOGFLAG="True"
export no_proxy="noproxy,localhost,127.0.0.1,${ip_address}"
...
## Section 1
This is a test document for dataprep validation.
It contains multiple sections to test chunking.

## Section 2
This section tests the embedding capabilities.
We want to verify that the text is properly split and embedded.
EOL
    ...
    if [ "$HTTP_STATUS" -eq 200 ]; then
        ...
        # Test get endpoint - NOTE: This is the fixed endpoint path
        echo "Testing get endpoint..."
        GET_RESPONSE=$(curl -s "http://localhost:$DATAPREP_PORT/v1/dataprep/get" | tee ${LOG_PATH}/dataprep_get.log)
        GET_STATUS=$?
...
function stop_docker() {
    ...
    for container in test-comps-dataprep-server test-comps-dataprep-tei-endpoint test-comps-arango; do
